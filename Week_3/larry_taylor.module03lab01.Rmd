...
two_sum <- function(nums_vector,target){
  for(i in (seq_along(nums_vector))){
    for(x in (seq_along(nums_vector))){
      if((nums_vector[i] + nums_vector[x]) == target){
        loop_work <- loop_work + 1
        vec1 <- c(vec1, paste(i, x, sep = " "))
...
two_sum <- function(nums_vector,target){
  ...
  for(m in (seq_along(h))){
     if( as.character((target - nums_vector[m])) %in% h[[as.character(m)]]){
       hash_work <- hash_work + 1
       vec2 <- c(vec2,paste((target - nums_vector[m]),nums_vector[m], sep=' '))
     }
